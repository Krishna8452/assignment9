...
const {
  ...
  isValidIdSchema,
  getUserSchema,
} = require("../modules/user/validation/validator");
 ...
 *         - in: body
 *           name: body
 *           description: Test data to be created (in JSON format)
 *           required: true
 *           schema: {}
 *       responses:
 *         '201':
...
router
  .route("/users/:id")
  .get(yupValidatorMiddleware(isValidIdSchema), getUser);
router
  .route("/users/:id")
  .put(yupValidatorMiddleware(updataUserSchema), editUser);

router.use(jwtAuthentication);
router
  .route("/users/:id")
  .delete(yupValidatorMiddleware(isValidIdSchema), deleteUser);
